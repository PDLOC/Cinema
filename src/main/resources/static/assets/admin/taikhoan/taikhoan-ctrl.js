app.controller("taikhoan-ctrl", function($scope, $http) {
	...
	$scope.create = function() {
		...
		$http.post(`/rest/customers`, item).then(resp => {
			...
			$scope.selection.forEach(r => {
				var authority = { taikhoan: item, vaitro: r };
	...
	$scope.update = function() {
		var item = angular.copy($scope.form);
		$http.put(`/rest/customers/${item.matk}`, item).then(resp => {
			var index = $scope.items.findIndex(p => p.matk == item.matk);
			$scope.items[index] = item;
			// Xóa toàn bộ roles của user hiện tại
			$http.delete(`/rest/authoritiesOne/${item.matk}`).then(resp => {
				$scope.selection.forEach(r => {
					var authority = { taikhoan: item, vaitro: r };
					$http.post(`/rest/authorities`, authority).then(resp => {
						$scope.items.push(resp.data);
